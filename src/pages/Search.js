...
import { searchProducts } from '../api/content';
...
const Search = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        setLoading(true);
        const results = await searchProducts(searchQuery, currentLanguage);
      ...
      <Box sx={{ py: 8, backgroundColor: '#f9f9f9', flex: 1 }}>
        <Container maxWidth="xl">
          ...
          {searchQuery && (
            ...
            >
              {t('search.results_for', 'Results for')}: <strong>"{searchQuery}"</strong>
            </Typography>
          )}
          ...
          {loading ? (
            ...
            >
              ...
              >
                {searchQuery 
                  ...
                  : t('search.enter_query', 'Enter a search term to find products')
                }
              </Typography>
              {searchQuery && (
                <Typography
            ...
            <>
              ...
              >
                {t('search.found_count', `Found ${products.length} products`)}
              </Typography>
              <Box
                sx={{
                  display: 'grid',
